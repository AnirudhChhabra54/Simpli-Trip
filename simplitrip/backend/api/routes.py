...
@router.get("/destinations/{destination}/full-details")
async def get_destination_full_details(destination: str):
    """
    📍 Get complete destination details from ChromaDB
    Returns full JSON data stored in ChromaDB including all travel information
    
    Parameters:
        - destination: Name of destination (e.g., "Ladakh", "Jaipur")
    
    Returns full destination object with all details from ChromaDB
    """
    try:
        ...
        logger.info(f"📍 Fetching full details for: {destination}")
        
        # Search for destination
        results = rag_service.retrieve(destination, top_k=3)
        
        if not results:
            logger.warning(f"❌ Destination not found in ChromaDB: {destination}")
            raise HTTPException(status_code=404, detail=f"Destination '{destination}' not found in knowledge base")
        
        # Find exact match or closest match
        exact_match = None
        for result in results:
            if result['meta']['destination'].lower() == destination.lower():
                exact_match = result
                break
        
        if not exact_match:
            exact_match = results[0]  # Use closest match
        
        meta = exact_match['meta']
        
        # Parse full JSON data
        try:
            full_data = json.loads(meta.get('data_json', '{}'))
        except:
            full_data = {}
        
        response_data = {
            "destination": full_data.get('destination', meta.get('destination')),
            "state": full_data.get('state', meta.get('state')),
            "region": full_data.get('region', meta.get('region')),
            "best_season": full_data.get('best_season', meta.get('best_season')),
            "climate": full_data.get('climate', meta.get('climate')),
            "budget": full_data.get('budget', meta.get('budget')),
            "attractions": full_data.get('attractions', []),
            "activities": full_data.get('activities', []),
            "food_specialties": full_data.get('food_specialties', []),
            "packing_list": full_data.get('packing_list', []),
            "travel_tips": full_data.get('travel_tips', []),
            "transportation": full_data.get('transportation', ''),
            "hotels": full_data.get('hotels', []),
            "source": meta.get('source', 'chromadb'),
            "is_custom": meta.get('custom', False),
            "last_updated": meta.get('timestamp', '')
        }
        
        logger.info(f"✅ Retrieved full details for {response_data['destination']}")
        ...
        logger.error(f"❌ Error fetching destination details: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/chromadb/status")
async def chromadb_health_status():
    """
    📊 Get ChromaDB health and statistics
    Shows if ChromaDB is active and number of documents indexed
    """
    try:
        ...
        # Count custom destinations
        try:
            all_docs = rag_service.collection.get(include=['metadatas'])
            custom_count = sum(1 for meta in all_docs.get('metadatas', []) 
                             if meta.get('custom') == True)
            predefined_count = stats.get('total_documents', 0) - custom_count
        except:
            custom_count = 0
            predefined_count = 0
        ...
        response = {
            ...
            "chromadb_active": is_available,
            "collection_name": stats.get('collection_name', 'unknown'),
            "total_documents": stats.get('total_documents', 0),
            "custom_destinations": custom_count,
            "predefined_destinations": predefined_count,
            "embedding_dimension": stats.get('embedding_dim', 0),
            "rag_service_version": "production",
            "message": "✅ ChromaDB is operational and serving knowledge base queries" if is_available else "❌ ChromaDB is not operational"
        }
        
        logger.info(f"✅ ChromaDB Status: {response['status']}")
        return response
