...
const apiClient = axios.create({
  ...
  timeout: 60000, // Increased to 60s to handle LLM generation times
});
...
// ============================================
// 1. LLM & PARSING ENDPOINTS (Stage 1)
// ============================================
...
// ============================================
// 2. SCRAPER & REAL-TIME DATA (Stage 2)
// ============================================

/**
 * Search destinations using web scraper (replaces old recommendation logic)
 * Endpoint: /api/v1/scraper/search-destinations
 */
export const searchDestinationsWithScraper = async (queryParams) => {
  try {
    const response = await apiClient.post('/api/v1/scraper/search-destinations', {
...
/**
 * Get detailed scraped information for a destination
 * Endpoint: /api/v1/scraper/destination-details
 */
export const getScrapedDestinationDetails = async (destination, queryParams) => {
  try {
    const response = await apiClient.post('/api/v1/scraper/destination-details', {
...
// ============================================
// 3. GENERATION ENDPOINTS (Stage 3)
// ============================================

/**
 * Generate complete day-by-day itinerary using AI + Scraped Data
 * Endpoint: /api/v1/itinerary/generate-complete
 */
export const generateCompleteItinerary = async (tripDetails) => {
  try {
    const response = await apiClient.post('/api/v1/itinerary/generate-complete', tripDetails);
    return response.data;
  } catch (error) {
...
/**
 * Generate description for itinerary (Optional helper)
 */
export const generateDescription = async (itinerary) => {
  try {
    const response = await apiClient.post('/api/v1/llm/generate-description', itinerary);
...
const aiService = {
  ...
  searchDestinationsWithScraper,
  getScrapedDestinationDetails,
  generateCompleteItinerary, // <--- New Critical Export
  generateDescription,
  explainRecommendation,
  getDestinationRecommendations,
  getNearbyRecommendations,
